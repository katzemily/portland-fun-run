...
Today we'll meet at `{r} current_venue$bar` in `{r} current_venue$quadrant` ([`{r} current_venue$address`](%60%7Br%7D%20current_venue$google_link%60)) for a `{r} current_venue$route_length` mile `{r} current_venue$route_wording`! We will meet around 6:55pm at `{r} current_venue$bar_abbr` and leave for the run at 7:05pm.

Don't forget your I.D! Also, if you have reflective gear or a light to run with, we recommend bringing that too.

Click here for the route: [`{r} current_venue$current_map`](%60%7Br%7D%20current_venue$current_map%60)
