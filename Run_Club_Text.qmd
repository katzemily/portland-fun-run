...
# Add row to historical table
history <- current_venue |> 
  mutate(effective_date = as_date(Sys.Date())) |> 
  rbind(history)

# Save out
save(history, file = "historical_data.Rda")
...
Today we'll meet at `{r} current_venue$bar` in `{r} current_venue$quadrant` ([`{r} current_venue$address`](`{r} current_venue$address`)) for a `{r} current_venue$route_length` mile `{r} current_venue$route_wording`! We will meet around 6:55pm at `{r} current_venue$bar_abbr` and leave for the run at 7:05pm.
